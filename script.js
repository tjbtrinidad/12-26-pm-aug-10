...
(function() {
    ...
    function initializeWebsite() {
        ...
        try { setupLoadingScreen(); } catch (e) { console.error('Setup loading screen failed:', e); }
        try { setupThemeToggle(); } catch (e) { console.error('Setup theme toggle failed:', e); }
        try { setupMobileMenu(); } catch (e) { console.error('Setup mobile menu failed:', e); }
        try { setupScrollEffects(); } catch (e) { console.error('Setup scroll effects failed:', e); }
        try { setupRevealAnimations(); } catch (e) { console.error('Setup reveal animations failed:', e); }
        try { setupContactForm(); } catch (e) { console.error('Setup contact form failed:', e); }
        try { setupSmoothScrolling(); } catch (e) { console.error('Setup smooth scrolling failed:', e); }
        try { setupFloatingCTA(); } catch (e) { console.error('Setup floating CTA failed:', e); }
        try { setupPremiumEffects(); } catch (e) { console.error('Setup premium effects failed:', e); }
        try { setupModal(); } catch (e) { console.error('Setup modal failed:', e); }
        try { setupCounterAnimations(); } catch (e) { console.error('Setup counter animations failed:', e); }
        
        // Portfolio and tools
        try { renderPortfolio(); } catch (e) { console.error('Render portfolio failed:', e); }
        try { initPortfolioFilters(); } catch (e) { console.error('Init portfolio filters failed:', e); }
        try { initPortfolioModal(); } catch (e) { console.error('Init portfolio modal failed:', e); }
        try { renderTools(); } catch (e) { console.error('Render tools failed:', e); }
        try { initWorkRoute(); } catch (e) { console.error('Init work route failed:', e); }
    ...
    function setupLoadingScreen() {
        ...
        // Emergency fallback - always hide after 4 seconds
        setTimeout(() => {
            hideLoader();
        }, 4000);
    }
    ...
    window.addEventListener('error', function(e) {
        ...
        hideLoader(); // Ensure loader is hidden on any JS error
    });
  ...
  { name: "Stripe", logo: "/public/img/tools/stripe.png" }
    ...
    function renderPortfolio(filter = "all") {
      ...
      items.forEach(p => {
        ...
        el.innerHTML = `
          <img src="${p.cover}" alt="${p.title}" loading="lazy" onerror="this.src='data:image/svg+xml;base64,'">
    ...
    function renderTools() {
      ...
      TOOLS.forEach(t => {
        ...
        div.className = "tool";
        div.innerHTML = `<img src="${t.logo}" alt="${t.name} logo" loading="lazy" onerror="this.src='data:image/svg+xml;base64,'">`;
...
/* ========= Liquid Plume Cursor (MNL-AI) ========= */
(function(){
  if (window.liquidCursor) return;

  const CFG = Object.assign({
    colors: [
      { stop: 0.00, rgba: [255,206, 82, 0.85] }, // mango core
      { stop: 0.25, rgba: [  0,198,108, 0.80] }, // emerald
      { stop: 0.55, rgba: [  0,224,198, 0.65] }, // electric teal
      { stop: 0.75, rgba: [122,248,225, 0.40] }, // icy teal
      { stop: 1.00, rgba: [  0,224,198, 0.00] }  // fade
    ],
    particles: 3,
    radius: 86,
    spring: 0.18,
    friction: 0.84,
    opacity: 0.32,
    trailFade: 0.08,        // lower = longer tail (0..1)
    mobileBreakpoint: 768
  }, window.LIQUID_CURSOR_CONFIG || {});

  const S = {
    enabled: true, started: false, hovering: false,
    pts: [], mouse:{x:innerWidth/2, y:innerHeight/2},
    dpr: 1, w: 0, h: 0, raf: 0, hidden: false
  };

  let canvas, ctx;

  function ensureCanvas(){
    canvas = document.getElementById('liquid-cursor') || Object.assign(document.createElement('canvas'), { id: 'liquid-cursor' });
  ...
  function resize(){
    const dpr = S.dpr = Math.min(devicePixelRatio || 1, 2);
    S.w = canvas.width  = Math.floor(innerWidth  * dpr);
    S.h = canvas.height = Math.floor(innerHeight * dpr);
    canvas.style.width  = innerWidth  + 'px';
    canvas.style.height = innerHeight + 'px';
  }

  function initPoints(){
    S.pts = Array.from({length: CFG.particles}, (_,i)=>({
      x: innerWidth/2, y: innerHeight/2, vx:0, vy:0,
      r: CFG.radius * (1 - i*0.12)
    }));
  }

  function onMove(e){ S.mouse.x = e.clientX; S.mouse.y = e.clientY; }
  function onHover(e){ S.hovering = !!(e.target && e.target.closest('a,button,input,textarea,select,[role="button"]')); }

  function rgba([r,g,b,a]){ return `rgba(${r},${g},${b},${a})`; }

  function drawBlob(p){
    const x = p.x * S.dpr, y = p.y * S.dpr, R = p.r * S.dpr;
    const g = ctx.createRadialGradient(x,y,0,x,y,R);
    for (const c of CFG.colors) g.addColorStop(c.stop, rgba(c.rgba));
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.arc(x,y,R,0,Math.PI*2); ctx.fill();
  }

  function stepPoint(p, mx, my){
    p.vx += (mx - p.x) * CFG.spring;
    p.vy += (my - p.y) * CFG.spring;
    p.vx *= CFG.friction; p.vy *= CFG.friction;
    p.x  += p.vx;         p.y  += p.vy;
  }

  function render(){
    // trail fade: overlay semi-transparent clear
  ...
  function loop(){
    if (!S.enabled || S.hidden) return;
    S.raf = requestAnimationFrame(loop);
    render();
  }

  const isReduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
  const isMobile = () => innerWidth < CFG.mobileBreakpoint;

  function start(){
    if (S.started) return; S.started = true;
    if (isReduced || isMobile()) { S.enabled = false; return; }
    if (!ensureCanvas()) { S.enabled = false; return; }
    resize(); initPoints();

    addEventListener('mousemove', onMove, { passive:true });
    addEventListener('resize', debounce(()=>{ resize(); if (isMobile()) api.disable(); else if (!S.enabled){ api.enable(); loop(); } }, 150));
    document.addEventListener('mouseover', onHover, { passive:true });
    document.addEventListener('mouseout', onHover, { passive:true });
    document.addEventListener('visibilitychange', ()=>{ S.hidden = document.hidden; if (!S.hidden && S.enabled) loop(); });

    loop();
  }

  const api = window.liquidCursor = {
    enable(){ if (S.enabled) return; S.enabled = true; loop(); },
    disable(){ S.enabled = false; cancelAnimationFrame(S.raf); },
    isEnabled(){ return S.enabled; },
    setConfig(partial){ Object.assign(CFG, partial||{}); resize(); }
  };
